...
export function set_up(user_overrides: Record<string, any>) {
  ...
  // Build final order directly (Python builds then reorders) for parity in result.
  const s: any[] = []
  ...
  s.push(new GcodeComment({ end_of_previous_line_text: '; including mesh bed level' }))
  ...
  e.push(new ManualGcode({ text: 'M900 K0 ; reset LA' }))
