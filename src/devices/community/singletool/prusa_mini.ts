...
export function set_up(user_overrides: Record<string, any>) {
  const printer_overrides = { primer: 'no_primer', nozzle_probe_temp: 170 }
  ...
  s.push(new ManualGcode({ text: 'G29 ; mesh bed leveling' }))
  ...
  s.push(new ManualGcode({ text: '; intro line\n' }))
  s.push(new ManualGcode({ text: '; intro line\n' }))
  s.push(new ManualGcode({ text: 'G1 X10 Z0.2 F1000\n' }))
  s.push(new ManualGcode({ text: 'G1 X70 E8 F900\n' }))
  s.push(new ManualGcode({ text: 'G1 X140 E10 F700\n' }))
  s.push(new ManualGcode({ text: 'G92 E0\n' }))
  ...
  e.push(new ManualGcode({ text: 'M221 S100 ; reset flow' }))
  e.push(new ManualGcode({ text: 'M572 S0 ; reset PA' }))
  e.push(new ManualGcode({ text: 'M593 X T2 F0 ; disable IS' }))
  e.push(new ManualGcode({ text: 'M593 Y T2 F0 ; disable IS' }))
